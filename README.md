...
ALTER TABLE Usuario_Evento ADD CONSTRAINT fk_usuario_evento_usu_email
  ...
  <p align="justify" style="font-family:roboto;">A lógica arquitetura implementada consistem em :</p>
        <p align="justify" style="font-family:roboto;">Model - Representação das tabelas do banco de dados, onde foi definido as relações entre objetos e seus atributos, foi utilizado o método ORM para mapear essas classes.</p>
	<p align="justify" style="font-family:roboto;">Repository - São interfaces Java que tem como principal função serem a camada de acesso a dados, durante seu desenvolvimento foi utilizado o padrão de projeto Facade pois é uma interface que simplifica as funcionalidades do JPA.  </p>
	<p align="justify" style="font-family:roboto;">Services - São classes que possuem os métodos do Repository, é onde são mantidos as lógicas conforme as regras de negócio conforme a requisição do cliente.</p>
	<p align="justify" style="font-family:roboto;">Controller - É a classe onde encontramos os endpoints utilizado para a interação com o front-end do projeto, a comunicação ocorre entre requisições HTTP presentes nas rotas do Controller, para essas requições utilizamos o padrão de projeto Proxy onde controlamos o acesso aos objetos nas com as anotações do Spring.</p>
  <details>
